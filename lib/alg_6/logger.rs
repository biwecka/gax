    ...
    for Logger
{
    fn log(
        ...
        let y_values: Vec<Vec<usize>> = population.iter().map(|(c, _)| {
        ...
        for y in y_values {
            let mut p: Vec<(usize, usize)> = x_values.iter().zip(y).map(|(a, b)| (*a, b)).collect();
            points.append(&mut p);
        }
        ...
        rec.log(
            "chromosomes",
            &ga::rerun::Points2D::new(map.iter().map(|((x, y), _)| (*x as f32, *y as f32)))
                .with_colors(map.iter().map(|((_, _), n)| {
                    // Calculate hue value (interval [0; 240])
                    let hue =
                        ((*n as f32 - min) * (240. - 0.)) / (max - min) + 0.;
