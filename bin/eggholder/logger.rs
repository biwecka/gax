...
impl Logger {
    pub fn draw_eggholder_function(&self) {
        ...
        let x0_values: Vec<f64> =
            (-512..512).step_by(2).map(|x| x as f64).collect();
        let x1_values: Vec<f64> =
            (-512..512).step_by(2).map(|x| x as f64).collect();
        ...
        self.rec
            .log(
                ...
                &rerun::Points2D::new(points.iter())
                    .with_colors(points.iter().map(|_| color)),
            )
            .unwrap();
    }
    ...
    pub fn log_success_rate(
        ...
        let _ =
            self.rec.log("success_rate/target", &Scalar::new(target as f64));
    }
