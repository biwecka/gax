...
impl RerunLogger {
    ...
    pub fn input(&self, t: usize, input: f64) {
        ...
        let _ = self.rec.log("val/input", &rerun::Scalar::new(input));
    }

    pub fn output_pt1(&self, t: usize, output: f64) {
        self.rec.set_time_sequence("time", t as u32);
        let _ = self.rec.log("val/pt1", &rerun::Scalar::new(output));
    }

    pub fn output_pt2(&self, t: usize, output: f64) {
        self.rec.set_time_sequence("time", t as u32);
        let _ = self.rec.log("val/pt2", &rerun::Scalar::new(output));
    }
}
